...
Apply these guidelines in this priority order: 1) safety and correctness, 2) readability and maintainability, 3) API clarity, 4) performance and optimization. Resolve conflicts by choosing the higher-priority item; if two lower-priority items conflict, prefer readability and maintainability unless that would significantly reduce API usability, and document the decision in a one-sentence comment. Examples: readability over performance, readability over API cleverness, or API clarity over micro-optimization.

For day-to-day coding, treat `General Instructions` and `Patterns to Avoid` as primary rules; use the remaining sections as supporting guidance.

## General Instructions

- Write readable and maintainable code by using clear names, consistent formatting, and small modular functions, without compromising safety and correctness.
...
- Structure async code using `async/await` and `tokio`. Don't use `async-std`.
...
- Avoid panics in any public-facing library code; use `Result` for error handling instead.
